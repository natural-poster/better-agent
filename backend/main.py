...
def _require_project_structure_internal(x_internal_token: str) -> None:
    ...
    if coordinator.principal_extension_id(x_internal_token) != extension_store.BUILTIN_PROJECT_STRUCTURE_EXTENSION_ID:
...
async def internal_project_update_counts_batch(
    ...
    return await asyncio.to_thread(
        lambda: {
            project_id: project_update_store.unseen_count(project_id)
...
async def get_session(
    ...
    root_id = await asyncio.to_thread(session_manager._root_id_for, session_id)
    barrier_seq = 0
    ...
    if isinstance(root_id, str):
        ...
        if has_events:
            ...
            # overshoots the on-screen tail. Handing the raw head dropped
            # the still-streaming turn on a mid-turn hard refresh — the
            # trailing non-render/unresolved events pushed the watermark
...
# ============================================================================
# Trace inspector internal substrate
# ============================================================================

def _verify_extension_internal_trace_call(x_internal_token: str) -> None:
    if not coordinator.is_internal_caller(x_internal_token):
        raise HTTPException(status_code=403, detail=t("error.invalid_internal_token"))
    if coordinator.principal_extension_id(x_internal_token) != extension_store.BUILTIN_TRACE_INSPECTOR_EXTENSION_ID:
        raise HTTPException(status_code=403, detail="trace inspector extension required")
    _require_builtin_runtime_extension(extension_store.BUILTIN_TRACE_INSPECTOR_EXTENSION_ID)
...
@app.post("/api/internal/traces/list")
async def internal_trace_list(
    body: dict,
    x_internal_token: str = Header(..., alias="X-Internal-Token"),
):
    _verify_extension_internal_trace_call(x_internal_token)
    if not isinstance(body, dict):
        raise HTTPException(status_code=400, detail="body must be an object")
    session_id = str(body.get("session_id") or "").strip() or None
    limit = _positive_int(body.get("limit"), 100, 1000)
    return {
        "traces": await asyncio.to_thread(
            trace_collector.list_traces,
            session_id=session_id,
            limit=limit,
        )
    }
...
@app.post("/api/internal/traces/get")
async def internal_trace_get(
    body: dict,
    x_internal_token: str = Header(..., alias="X-Internal-Token"),
):
    _verify_extension_internal_trace_call(x_internal_token)
    ...
    trace = await asyncio.to_thread(trace_collector.get_trace, trace_id)
    if not trace:
        return {"error": t("error.trace_not_found")}
    return trace


@app.post("/api/internal/traces/search")
async def internal_trace_search(
    body: dict,
    x_internal_token: str = Header(..., alias="X-Internal-Token"),
):
    _verify_extension_internal_trace_call(x_internal_token)
    ...
    limit = _positive_int(body.get("limit"), 50, 1000)
    return {
        "traces": await asyncio.to_thread(
            trace_collector.search_traces,
            query,
            limit=limit,
        )
    }
...
@app.post("/api/internal/traces/grep")
async def internal_trace_grep(
    body: dict,
    x_internal_token: str = Header(..., alias="X-Internal-Token"),
):
    _verify_extension_internal_trace_call(x_internal_token)
    ...
    if not query:
        raise HTTPException(status_code=400, detail="q is required")
    field = str(body.get("field") or "all").strip()
    session_id = str(body.get("session_id") or "").strip() or None
    step_type = str(body.get("step_type") or "").strip() or None
    limit = _positive_int(body.get("limit"), 50, 1000)
    return {
        "matches": await asyncio.to_thread(
            trace_collector.grep_traces,
            query,
            field=field,
            session_id=session_id,
            step_type=step_type,
...
@app.post("/api/internal/traces/stats")
async def internal_trace_stats(
    body: dict,
    x_internal_token: str = Header(..., alias="X-Internal-Token"),
):
    _verify_extension_internal_trace_call(x_internal_token)
    ...
    session_id = str(body.get("session_id") or "").strip() or None
    return await asyncio.to_thread(
        trace_collector.get_trace_stats,
        session_id=session_id,
    )


@app.post("/api/internal/traces/latest")
async def internal_trace_latest(
    body: dict,
    x_internal_token: str = Header(..., alias="X-Internal-Token"),
):
    _verify_extension_internal_trace_call(x_internal_token)
    ...
    trace = await asyncio.to_thread(
        trace_collector.get_latest_trace,
        session_id=session_id,
    )
    if not trace:
        return {"error": t("error.traces_not_found")}
    return trace
